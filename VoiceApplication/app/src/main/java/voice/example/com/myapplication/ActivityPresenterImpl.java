...
public class ActivityPresenterImpl implements ActivityContract.IActivityPresenter, TcpClient.ConnectListener {
    ...
    private final int STOP_RECORD = 5;
    ...
    public void onConnected(boolean success) {
        if (success) {
            updateConnectState(ActivityContract.SpeakType.STOP);
        } else {
            updateConnectState(ActivityContract.SpeakType.START);
        }
    }
    //socket client 回调
    @Override
    ...
    @Override
    public void playCurRecorder() {
        if (mIsRecording) {
            Toast.makeText(mContext, "正在录音", Toast.LENGTH_LONG).show();
    ...
    public void next() {
        ...
        if (mIsRecordStop) {
            Toast.makeText(mContext, "请点击开始录音", Toast.LENGTH_LONG).show();
            mActivityView.onShowStopView();
            return;
        }
        if (mCurRecordItem != null && mNextRecordItem == null) {
    ...
    private void updateConnectState(int type) {
    ...
    private void onRecordSound() {
        if (mCurRecordItem == null) return;
        startSoundRecord();
        mHandler.sendEmptyMessageDelayed(STOP_RECORD, DELAY_TIME);
    }
    ...
    private void recordPlayStart() {
        ...
        if (mCurRecordFile == null || !mCurRecordFile.exists()) {
            Toast.makeText(mContext, "当前录音文件不存在,请点击开始录音", Toast.LENGTH_LONG).show();
            mActivityView.onPlayFinishedView();
            return;
        }
        ...
        mExecutor.submit(new Runnable() {
            ...
            public void run() {
                ...
                int minBuffSize = AudioTrack.getMinBufferSize(16000/2,
                        AudioFormat.CHANNEL_CONFIGURATION_STEREO, AudioFormat.ENCODING_PCM_16BIT);
                final byte[] mBuffer = new byte[minBuffSize];
                mAudioTrack = new AudioTrack(AudioManager.STREAM_MUSIC, 16000/2,
                        AudioFormat.CHANNEL_CONFIGURATION_STEREO, AudioFormat.ENCODING_PCM_16BIT, minBuffSize,
